...
Sub Main
    ...
    Do
		message.Speak "Veuillez entrer l'heure de l'alerte"
        ...
        If userTimeInput = "" Then ' Si l'utilisateur annule la saisie
			message.Speak "Aucune heure entrée, le programme va s'arrêter."
		...
		message.Speak "Format d'heure invalide, veuillez réessayer."
        ...
        ElseIf Now > alertTime Or DateDiff("n", Now, alertTime) < 5 Then
			message.Speak "L'heure entrée doit être au moins 5 minutes plus tard que l'heure actuelle."
    ...
    Do While Now < nowAlertTime'alertTime
        If Now >= preAlertTime Then
			...
			message.Speak "Ceci est un rappel! Il reste maintenant " & minutesRemaining & "minutes avant l'heure de votre alerte."
        ...
        End If
    Loop
		message.Speak "L'heure de l'alerte est passée. Veuillez entrer une nouvelle heure pour la prochaine alerte"
...
Sub Sortir()
				message.Speak "Une autre instance du programme est détectée. Ce programme va s'arrêter après 10 secondes."
